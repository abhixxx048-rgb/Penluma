---
title: 'Stocks and Flows: What Builds Up vs. What Moves'
metaTitle: 'Stocks and Flows: The Bathtub Rule Explained'
description: >-
  Stocks and flows explain why debt isn't the deficit and savings grow slowly.
  Learn the bathtub rule, the time-stop test, and stop confusing the two for good.
keywords:
  - stocks and flows
  - what is a stock and a flow
  - stock vs flow
  - debt vs deficit
  - income vs wealth
  - systems thinking
  - bathtub model systems
  - dynamic equilibrium
  - technical debt
  - policy resistance
  - Donella Meadows stocks and flows
  - system dynamics
...
faq:
  - q: What is the difference between a stock and a flow?
    a: A stock is an accumulation you can measure at a single moment, like the water in a bathtub or money in your account. A flow is the rate at which that stock fills or drains, measured per unit of time, like the tap running or your monthly paycheck.
  - q: What is the difference between debt and the deficit?
    a: Debt is a stock, the total amount owed right now. The deficit is a flow, how much the debt grows in one year. As long as the deficit is positive, the debt keeps growing, just more slowly. Cutting the deficit does not shrink the debt.
  - q: How can you quickly tell a stock from a flow?
    a: Use the time-stop test. Imagine you freeze time. If the quantity still exists when the clock stops, it is a stock. If it vanishes because it needs time to happen, it is a flow.
  - q: Why do stocks change so slowly?
    a: Flows take time to flow, so a large stock has inertia, like a freight train that keeps rolling after the engine cuts off. This is why halving emissions tomorrow would not cool the planet quickly, and why savings grow gradually.
  - q: Is trust a stock or a flow?
...
3. If inflow equals outflow, the stock **holds steady**. This calm balanced state is called **dynamic equilibrium**.
...
1. **Run the time-stop test on any number you hear.** Freeze the clock. Does it survive? Stock. Does it vanish? Flow. Do this before you draw any conclusion.
2. **Check the units.** A bare quantity (dollars, people) is a stock. A "per time" rate (dollars per year, hires per quarter) is a flow. Mismatched units are where the confusion lives.
3. **When you want to change a stock, ask about both valves.** Not just "how do I raise the inflow?" but also "how do I lower the outflow?" There is always more than one way to fill a bathtub.
4. **Respect the inertia.** Before expecting fast results, ask how big the stock is relative to its flows. Big stock, small flow means slow change, so plan for the lag instead of pushing the lever harder.
5. **Protect your slow-build stocks defensively.** Trust, reputation, and institutional knowledge take years to fill and days to drain. Guard the outflow, do not just chase the inflow.
6. **Name the stock and the flow out loud in any debate.** "Are we talking about the debt or the deficit? The wealth or the income?" Half of bad arguments dissolve the moment you separate the two.
