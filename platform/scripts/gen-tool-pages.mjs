...
const CONTENT = {
  ...
  'bulk-multi-file-cleaner': {
    eyebrow: 'Power tool · Free · In your browser',
    title: 'Bulk / Multi-File Cleaner — Clean Many Files at Once',
    description:
      'Drop multiple text files, clean each one in your browser, and download the results in a batch. No upload, no signup. Free bulk text cleaner, in-browser.',
    keywords: ['bulk text cleaner', 'clean multiple files', 'batch text cleanup', 'multi file cleaner'],
    lede:
      'Drop <strong>multiple files at once</strong>, clean each one in your browser, and download the results in a batch — no upload, no queue.',
    definition:
      'A bulk cleaner applies the same text-cleanup pass to many files at once, entirely on your device.',
    steps: [
      'Drag in the files you want to clean.',
      'Pick the cleanup options to apply to all of them.',
      'Download the cleaned files individually or together.',
    ],
    faqs: [
      { q: 'How do I clean multiple files at once?', a: 'Drop them all onto the tool. Each file is read and cleaned in your browser with the options you choose, then offered back for download — nothing is uploaded to a server.' },
      { q: 'What file types are supported?', a: 'Plain-text formats like .txt and .md work best. The cleaner reads the text content in your browser and applies the same pass you’d run on a single paste.' },
      PRIVACY_FAQ,
    ],
    featureList: ['Batch multiple files', 'Same cleanup options across all', 'Per-file download', 'Nothing leaves the browser'],
  },

  'on-device-ai-rewrite': {
    eyebrow: 'On-device AI · Free · In your browser',
    title: 'On-Device AI Rewrite — Tone & Length Rewrites in Your Browser',
    description:
      'One-click tone and length rewrites using your browser’s built-in AI — your text never leaves your device. Free on-device AI rewriter, experimental.',
    keywords: ['on device ai', 'browser ai rewrite', 'private ai rewrite', 'built-in ai rewriter'],
    lede:
      'One-click <strong>tone and length rewrites</strong> using your browser’s built-in AI — so the text never leaves your device.',
    definition:
      'On-device AI rewrite uses a language model that runs inside your browser to rephrase text without sending it to any server.',
    steps: [
      'Paste the passage you want to rewrite.',
      'Choose a tone or length target.',
      'Review the rewrite side by side and keep what works.',
    ],
    faqs: [
      { q: 'Does my text get sent to a server?', a: 'No. This tool uses your browser’s built-in, on-device AI, so the rewrite happens locally. If your browser doesn’t support it yet, the tool tells you instead of falling back to a cloud service.' },
      { q: 'Which browsers support built-in AI?', a: 'Support is new and rolling out gradually (notably in recent Chrome builds behind their built-in AI features). Where it isn’t available, try the WebGPU rewrite, which loads a model into the page instead.' },
      PRIVACY_FAQ,
    ],
    featureList: ['Uses browser built-in AI', 'Tone and length presets', 'Side-by-side review', 'No cloud upload'],
  },

  'webgpu-webllm-rewrite': {
    eyebrow: 'On-device AI · Free · In your browser',
    title: 'WebGPU Rewrite — Experimental On-Device Rewrite That Never Uploads',
    description:
      'Run a language model directly in your browser with WebGPU to rewrite text locally — nothing is uploaded. Free, experimental on-device rewriter.',
    keywords: ['webgpu llm', 'in browser llm', 'webllm rewrite', 'private ai rewrite'],
    lede:
      'Load a language model <strong>directly into the page with WebGPU</strong> and rewrite text locally — your words never leave the browser.',
    definition:
      'A WebGPU rewrite runs an actual LLM on your device’s GPU inside the browser, with no server round-trip.',
    steps: [
      'Let the model download and initialize (one time, cached).',
      'Paste your text and choose how to rewrite it.',
      'Review the local rewrite — nothing was uploaded.',
    ],
    faqs: [
      { q: 'What is an in-browser LLM with WebGPU?', a: 'WebGPU lets web pages use your GPU, which makes it possible to run a compact language model directly in the browser. The model weights download once and cache; after that, rewrites run locally with no server involved.' },
      { q: 'Why is it slower the first time?', a: 'The model has to download and initialize on first use, which can take a moment and a few hundred MB. Subsequent runs use the cached model and are much faster.' },
      PRIVACY_FAQ,
    ],
    featureList: ['Runs an LLM via WebGPU', 'Model cached after first load', 'Fully local inference', 'Nothing uploaded'],
  },
};
