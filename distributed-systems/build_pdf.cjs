/* Build the Distributed Systems Foundations study guide HTML from the workflow output. */
const fs = require('fs');
...
const sectionHtml = sections
  .map((s) => s.html.replace('<h2', `<h2 id="sec-${s.num}"`))
  .join('\n');
...
const toc = sections
  .map((s) => `<li><a href="#sec-${s.num}">${s.num}. ${s.title.replace('&', '&amp;')}</a></li>`)
  .join('\n');
...
dl{ margin:1em 0; }
dt{ font-weight:700; color:#173a63; margin-top:.7em; }
...
<section class="cover">
  <div class="kicker">Study Guide · Series 1 of the Distributed Systems track</div>
  <h1>Distributed Systems<br>Foundations</h1>
  <div class="rule"></div>
  <p class="sub">A Beginner-to-Advanced Study Guide<br>From "what is a distributed system?" to consistency models</p>
...
${sections.map((s) => `<section class="section">${s.html.replace('<h2', '<h2')}</section>`).join('\n')}
...
const htmlPath = path.join(OUT_DIR, 'Distributed-Systems-Foundations.html');
fs.writeFileSync(htmlPath, html, 'utf8');
console.log('HTML written:', htmlPath, '(' + html.length + ' chars)');
