/* Builder: assembles the Systems Thinking study-guide HTML from the workflow output JSON.
 * Usage: node build_guide.cjs <output.json> <out-basename> "<title|with|pipes>" "<subtitle>" "<kicker>"
 * Then render to PDF:
...
const toc = sections
  .map((s) => {
    ...
    if (s.part && s.part !== lastPart) {
      lastPart = s.part;
      prefix = `<div class="group">${esc(s.part)}</div>\n`;
    }
    return `${prefix}<li><a href="#sec-${s.num}">${s.num}. ${esc(s.title)}</a></li>`;
  })
  .join('\n');
...
h3{ font-size:14pt; margin:1.5em 0 .5em; color:#0b4f49; }
...
dl{ margin:1em 0; } dt{ font-weight:700; color:#0b4f49; margin-top:.7em; }
