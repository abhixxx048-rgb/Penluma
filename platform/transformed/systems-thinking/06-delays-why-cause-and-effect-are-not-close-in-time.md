---
title: "Delays: Why Cause and Effect Aren't Close in Time"
metaTitle: "Delays in Systems: Why Cause and Effect Lag"
description: "Why do systems overshoot, oscillate, and crash? A hidden delay between cause and effect is usually the culprit. Here's how to spot it and act before it bites."
keywords:
  - delays in systems thinking
  - cause and effect not close in time
  - bullwhip effect
  - feedback loop delay
  ...
  - shower temperature delay analogy
  - systems thinking delays Meadows
...
faq:
  ...
  - q: Why do delays cause oscillation?
    a: By the time feedback finally arrives, you've usually already corrected too much. Your action was calibrated to a problem that has since reversed, so you overshoot one way, then overcorrect the other way, again and again.
  - q: Why does reacting faster often make things worse?
    a: When the delivery delay is long, reacting fast just piles new corrections on top of earlier ones that haven't landed yet. The corrections all arrive together and create a bigger overshoot. Small, patient adjustments work better.
  - q: What is the bullwhip effect?
    a: The bullwhip effect is when small swings in customer demand get amplified into huge swings further up a supply chain. It comes from delays and from re-ordering while ignoring goods already in transit, not just from irrational people.
  - q: How do you manage long-delay systems?
    a: Use foresight. Watch leading indicators instead of waiting for proof, count what's already "in flight" before correcting again, and make small, deliberate moves rather than fast reactive ones.
sources:
...
1. **Perception delay** is the time it takes to *notice and trust* a change. A shop owner might average five days of sales before believing a trend is real, so today's decision rests on five-day-old data.
2. **Response delay** is the gap between recognizing a problem and acting on it. Often it's deliberate: instead of fixing a shortfall all at once, you spread the correction over several steps.
3. **Delivery delay** is the time between taking action and the result actually arriving, like a supplier's shipping time, a crop's growing season, or a new hire's ramp-up.
...
1. **Name the three delays.** How long until you'll *notice* the effect (perception)? How long between deciding and acting (response)? How long until the action actually lands (delivery)? Writing them down stops you from assuming feedback is instant.
2. **Count what's in flight.** Before you correct again, subtract everything you've already set in motion that hasn't arrived. Most over-ordering and over-correcting comes from forgetting the pipeline.
3. **Slow down when delivery is slow.** If the result takes a long time to land, make one small adjustment and *wait* for it before making another. Resist the urge to push harder just because nothing's happened yet.
4. **Watch leading indicators, not lagging ones.** Track the early signs of what's coming, not the proof that it already happened. Lagging data tells you where you've been.
5. **Build in foresight.** As Meadows writes, "to act only when a problem becomes obvious is to miss an important opportunity to solve the problem." For slow systems, act before the effect is visible.
6. **Shorten the delivery delay where you can.** Faster shipping, shorter production cycles, and modest buffer stock all reduce how badly the system can swing.
