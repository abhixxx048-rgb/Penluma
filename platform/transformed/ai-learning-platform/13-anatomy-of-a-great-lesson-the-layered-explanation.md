---
title: 'Why One Explanation Fails (and Layered Lessons Win)'
metaTitle: 'How to Explain Anything: The Layered Lesson'
description: >-
  Learn the layered explanation method that makes ideas stick: simple, deeper,
  analogy, example, real use, and practice. A clearer way to teach and learn.
...
faq:
  - q: What is a layered explanation?
    a: >-
      A layered explanation teaches one idea through several complementary
      passes instead of a single phrasing: a simple version, a deeper one, an
      analogy, a worked example, a real-world use, and finally practice. Each
      layer offers a different doorway so understanding can catch on whichever
      one fits the learner.
  - q: Are people really visual or verbal learners?
    a: >-
      No. The popular idea that matching a "learning style" to a person improves
      results is not supported by research. Everyone benefits from seeing an idea
      from multiple angles, regardless of a supposed style.
  - q: Why is practice more powerful than re-reading?
    a: >-
      Pulling information out of memory, through a quiz or explaining it back,
      strengthens learning far more than reading it in again. This is called the
      testing effect, and it is the most underused tool in weak lessons.
  - q: What is working memory and why does it matter for learning?
    a: >-
      Working memory is the small mental workbench where you hold what you are
      thinking about right now. It only holds about four items and empties in
      seconds, so good lessons deliver ideas in small pieces rather than a wall
      of text.
  - q: Should every lesson use all six layers?
    a: >-
      No. The layers are a menu, not a mandate. A confused beginner may need all
      six, while an advanced learner may only need the deeper explanation plus
      practice. Forcing simple layers on an expert wastes attention.
  - q: What is the Feynman technique?
    a: >-
      It is the practice of explaining an idea in plain language as if teaching a
      beginner. If you cannot explain it simply, you have found the exact gap in
      your understanding to go back and fix.
sources: []
---
...
1. **Simple** — a plain, one-line "what is it."
2. **Deeper** — the precise, technical version.
3. **Analogy** — map it onto something familiar.
4. **Example** — one concrete, worked-out case.
...
1. **Name the one idea.** A lesson teaches a single concept. If you have three, you have three lessons.
2. **Open simple.** Write one plain sentence first, with no jargon. If you can't, you don't yet understand it.
3. **Then go precise.** Add the accurate, technical version, defining each new term before using it.
4. **Bridge with an analogy** to something the listener already knows — and name where the analogy breaks.
5. **Show a worked example** all the way through before asking anyone to solve anything alone.
6. **Anchor it to real life** so the learner sees when they'd use it.
7. **End with retrieval.** Ask a question, request a prediction, or have them explain it back. Never finish on "does that make sense?" — finish on "show me."
8. **Cut, then cut again.** Drop the gentle layers for advanced learners, and delete anything decorative.
