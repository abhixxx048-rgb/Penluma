...
> **Date:** 2026-06-16
> **Companion to:** [`readme/ACQUISITION_CHANNELS_2026-06-15.md`](./ACQUISITION_CHANNELS_2026-06-15.md) (the *why*). This doc is the *how* - copy-paste-ready scripts, lists, page outlines, and trackers so a founder can start on Monday.
> **Scope:** The two primary bets only. **BET 1** = founder-led manual outreach fused with 2 print communities. **BET 2** = bottom-of-funnel SEO + a free standalone design tool. Everything here is manual, near-zero-cash, and built to throw off the messaging the rest of the funnel reuses.
...
- **Google Maps** - search `screen printing`, `sign shop`, `print shop`, `embroidery`, `promotional products` in target metros. Each result has phone, site, reviews.
- **Yelp / Yellow Pages** - same categories; good for filtering by "no website".
- **Trade directories** - Printing United Alliance / former SGIA member directory, NAQP, local print-association member lists.
- **Instagram** - hashtags `#screenprinting #customtshirts #signshop #printshop`; DM-able owners who post their work but link to a Linktree, not a store.

**Qualify each row (the filter that matters):**
1. Do they have a website? (no site = stronger pain, but slower to close)
2. Can a customer **order or upload artwork online** right now? (if no → ideal)
3. Is there a visible owner/name to personalize to?
4. Any sign of size (reviews count, team photos) - favor 1–10 person shops.
...
**Your two to OWN:**
1. **r/SCREENPRINTING** (large, active, owner-heavy) - public, searchable, answers index in Google.
2. **"Screen Printing Q&A" private Facebook group** (~14k, pro-skewed) - request to join, answer the "how do I take orders online / what software" threads.
...
- Set free saved searches for: `online ordering`, `artwork upload`, `web to print`, `storefront`, `take orders online`, `proof approval`, `what software`.
- Answer those threads **product-agnostically and genuinely**. Mention Print-Flow-360 only when it's literally the best answer to the exact question - and disclose you built it.
...
- Most common `already-use-X` → tells you which **comparison pages to build first** in Bet 2 (§7).
- Repeated `missing-feature` → product roadmap signal.
- The exact *words* people use → your homepage headline, onboarding copy, and BOFU page intros. Use their language, not yours.
...
Priority order:
1. **5–10 comparison / "alternatives" pages** (§7) - highest intent, lowest competition, directly answers `already-use-X`.
2. **The free standalone design tool** (§8) - engineering-as-marketing magnet + live product demo.
3. **3–5 JTBD guides** (§9) - written in the exact objection language.
...
1. One-line answer up top (who it's best for).
2. Honest comparison table (features, who-it's-for, pricing transparency).
3. "Where [Competitor] is strong" - builds trust.
4. "Where Print-Flow-360 fits better" - your wedge (non-technical owners, fast setup, visual studio).
5. Embedded live demo store + studio screenshots.
6. Single CTA: dedicated landing page with the same Loom-style demo.
...
- SEO target: `free business card maker`, `free flyer maker`, `online [product] designer` (high volume; the gate converts a slice to product trials).
- Instrument: every export and account-create gets a UTM/source so you can read tool → trial conversion.
...
- **Don't** chase `print on demand` / `how to start a print on demand business` - owned by Printful/Printify/Gelato/Shopify/Wix, unwinnable for 1–2+ years.
- Mirror each money page with a single-CTA landing page + live demo.
