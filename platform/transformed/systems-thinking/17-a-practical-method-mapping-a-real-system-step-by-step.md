---
title: 'How to Map a System: An 8-Step Method That Actually Works'
metaTitle: 'How to Map a System: 8-Step Method'
description: >-
  Learn how to map a system step by step with a repeatable 8-step method that
  finds the one change that actually shifts behavior, not the dozen that feel busy.
...
faq:
  - q: What is a system map and why would I make one?
    a: A system map is a diagram of the stocks, flows, feedback loops, and delays that produce a recurring problem. Its job is to reveal the one or two changes that will actually shift behavior, and to warn you off the many fixes that feel productive but change nothing.
  - q: What is a reference mode in systems thinking?
    a: A reference mode is a rough sketch of how a key quantity has changed over time and how you fear it will keep changing. It moves you from event thinking ("the queue was long Tuesday") to pattern thinking ("waits grew from 4 to 18 minutes over six months").
  - q: What is the difference between a stock and a flow?
    a: A stock is an accumulation you could still count if time froze, like water in a tub or money in an account. A flow is a rate measured per unit of time, like customers per hour. Use the time-stop test, if you can still count it with all activity stopped, it is a stock.
...
4. Elevate it (invest to expand it).
5. Go back to step 1.
...
1. Am I changing structure, or just symptoms?
2. Does this create a new dependency?
3. Who else reacts to this change?
4. What happens to the stocks I did not target?
...
1. **Draw a behavior-over-time graph.** Name the problem as a trend, not a single event.
2. **Set the boundary** on purpose: endogenous, exogenous, excluded. Write down what you leave out.
3. **List the key stocks** using the time-stop test.
4. **Find the flows** for each stock: what fills it, what drains it.
5. **Draw the causal links** with polarity. Mark every reinforcing and balancing loop, and every delay.
6. **Identify the dominant loop right now** and the binding constraint.
7. **Locate the leverage points**, and reach for the moderate-to-strong ones (information, rules, goals) over weak parameter tweaks.
8. **Trace second-order effects** around the loops before you choose anything.
